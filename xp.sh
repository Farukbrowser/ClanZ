...
Name=$(curl -sS https://raw.githubusercontent.com/Farukbrowser/ClanZ/main/Ipmini | grep $MYIP | awk '{print $2}')
...
licensi () {
    MYIP=$(curl -sS https://ipinfo.io/ip)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Farukbrowser/ClanZ/main/Ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
    res="licensi Denied!"
